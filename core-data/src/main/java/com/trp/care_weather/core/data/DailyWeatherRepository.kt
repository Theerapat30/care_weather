...
interface DailyWeatherRepository {
    val dailyWeathers: Flow<List<String>>

    suspend fun add(name: String)
}

class DefaultDailyWeatherRepository @Inject constructor(
    private val dailyWeatherDao: DailyWeatherDao
) : DailyWeatherRepository {

    override val dailyWeathers: Flow<List<String>> =
        dailyWeatherDao.getDailyWeathers().map { items -> items.map { it.name } }

    override suspend fun add(name: String) {
        dailyWeatherDao.insertDailyWeather(DailyWeather(name = name))
    }
}
